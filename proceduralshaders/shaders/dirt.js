...
const vec3 brown = vec3(.2, .1, .0);
const vec3 darkbrown = vec3(.1, .05, .0);

void main() {
    float noiseTexture = fbm(15. * v_position);
    noiseTexture = smoothstep(.3, .65, noiseTexture);
    vec3 COLOR = mix(brown, darkbrown, noiseTexture);
